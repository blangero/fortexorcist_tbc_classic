...
local function CD_ScanBookCooldowns()
  ...
  -- while true do -- NOT WORKING AT ALL
    ...
    -- if spell then
      -- spell,texture = FW:SpellName(spell);
	  --FW:Show(spell);
...
local function CD_ScanBuffCooldowns()
  ...
  local flag;
  while true do
    local spell,_,texture,count,_,duration,expirationTime = UnitAura("player",i);
    if spell then
      -- hidden cd code
      if hiddencooldown[spell] then
        CD:CheckCooldown(spell,expirationTime-duration,hiddencooldown[spell],texture,FLAG_INTERNAL);
      end
      -- normal buff code
      flag = CD_IsSpecialCooldown(spell,FLAG_BUFF);
      if flag then
        CD:CheckCooldown(spell,expirationTime-duration,duration,texture,flag);
      end
      i=i+1;
    else
      break;
  ...
  i = 1;
  while true do
    local spell,_,texture,count,_,duration,expirationTime = UnitAura("player",i,"HARMFUL");
    if spell then
      -- hidden cd code
      if hiddencooldown[spell] then
        CD:CheckCooldown(spell,expirationTime-duration,hiddencooldown[spell],texture,FLAG_INTERNAL);
      end
      CD:CheckCooldown(spell,expirationTime-duration,duration,texture,CD_IsSpecialCooldown(spell,FLAG_DEBUFF) or FLAG_DEBUFF);
      i=i+1;
    else
      break;
    end
...
local function CD_CastFailed(event,unit,num,spellID) -- items that are on cooldown trigger a fail of their effects
...
local function NewIcon(parent,n)
  local icon;
  parent.icons[n] = CreateFrame("Frame",nil,parent, BackdropTemplateMixin and "BackdropTemplate");
  icon = parent.icons[n];
  icon.parent = parent;
...
local function NewBar(parent,n)
  local bar;
  parent.bars[n] = CreateFrame("Frame",nil,parent, BackdropTemplateMixin and "BackdropTemplate");
  bar = parent.bars[n];
  bar.parent = parent;
  ...
  -- icon
  bar.icon = CreateFrame("Button",nil,bar, BackdropTemplateMixin and "BackdropTemplate");
...
local function CD_NewCooldownFrame(index)
  ...
  frame.tagsframe = CreateFrame("Frame",nil,frame, BackdropTemplateMixin and "BackdropTemplate");
  frame.tagsframe:SetAllPoints(frame);

  frame:SetPoint("CENTER",UIParent,"CENTER", 922, 100);
  frame:SetHeight(20);
