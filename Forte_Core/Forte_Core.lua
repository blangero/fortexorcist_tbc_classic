...
do -- update frame related stuff uses locals with the smallest scope possible
  local FC_UpdateFrame = CreateFrame("Frame", nil, nil,BackdropTemplateMixin and "BackdropTemplate");
  local FW_Events = {};
  local FW_Updated = {};
  ...
  FW.UnitAura = function(unit,buff,r,filter) -- works exactly like the original, but takes custom names into account
    if type(buff) == "string" then
      ...
      if id then
        local i = 1;
        while true do
          local name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff, value1, value2, value3 = origUnitAura(unit,i,filter);
          if name then
            if id == spellId then
              return buff, 1, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff, value1, value2, value3;
            end
            i=i+1;
          else
            return nil;
      ...
      local name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff, value1, value2, value3 = origUnitAura(unit,buff,r,filter);
      return CustomIDToName[spellId] or name, 1, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff, value1, value2, value3;
    end
  end
